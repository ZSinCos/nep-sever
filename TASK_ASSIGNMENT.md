...
- 环形饼图（内径45%，外径70%）
- 配色方案：优(深蓝#2E3A87)、良(绿#4CAF50)、轻度(黄#FFC107)、中度(紫粉#E040FB)、重度(亮蓝#2196F3)、严重(青绿#00BCD4)
- 底部图例，悬浮显示数据
```
...
- 中国地图着色图（ECharts map类型），注册china GeoJSON
- visualMap：颜色从绿(低)渐变到红(高)
- 省份标注，悬浮显示超标次数
```
...
```
标题：空气质量检测实时统计
三列横向排列，亮青色(#00E5FF)数字：
1. 检测总数量 - fa-users图标
2. 良好数量 - fa-sun-o图标
3. 污染数量 - fa-shield图标（计算：总数-良好数）
```
