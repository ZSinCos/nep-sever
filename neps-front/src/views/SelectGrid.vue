      ...
      <div style="background-color: #FFF; padding: 4vw; border-radius: 2vw; margin-bottom: 3vw;">
        ...
        <table class="table-form" style="margin: 0;">
          <tr>
            <td>省</td>
            <td>
              <select v-model="selectedProvinceId" @change="provinceChange">
                <option value="">请选择省</option>
                <option v-for="item in provinceList" :key="item.provinceId" :value="item.provinceId">{{ item.provinceName }}</option>
              </select>
            </td>
          </tr>
          <tr>
            <td>市</td>
            <td>
              <select v-model="selectedCityId" @change="cityChange">
                <option value="">请选择市</option>
                <option v-for="item in cityList" :key="item.cityId" :value="item.cityId">{{ item.cityName }}</option>
              </select>
            </td>
          </tr>
        </table>
...
import { removeSessionStorage } from '../common';
...
const provinceChange = async () => {
  ...
  if (selectedProvinceId.value) {
    selectedProvince.value = provinceList.value.find(p => p.provinceId === selectedProvinceId.value);
...
const cityChange = async () => {
  if (selectedProvinceId.value && selectedCityId.value) {
    selectedCity.value = cityList.value.find(c => c.cityId === selectedCityId.value);
